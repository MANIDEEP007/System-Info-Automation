'''Module to Get System information of the Linux System'''
import subprocess
from fpdf import FPDF as pdf

def decoder(byte_obj):
    '''Method to decode byte objects to String'''
...
def public_ip_get():
    '''Method to get the public IP of the system'''
...
def kernel_version():
    '''Method to find kernel version of the system'''
...
def java_version_checker():
    '''Method to find the version of the Java'''
    try:
...
def cpu_info():
    '''Module to Get information regarding CPU'''
    cpu_info_list = []
...
def pdf_info():
    '''Method to prepare PDF containing system information'''
    #Result Variables
    ...
    #End of Result Variables

    pdf_doc = pdf()
    pdf_doc.add_page()
    pdf_doc.set_text_color(0, 0, 255)
    pdf_doc.set_font('Arial', 'B', 12) #Second Argument Font Style B- Bold, I- Italic
    pdf_doc.write(1.3, 'Linux System Information\n')# First Argument - Line Height
    pdf_doc.line(8, pdf_doc.get_y()+5, pdf_doc.w-5, pdf_doc.get_y()+5) #Start Point, End Point
    #pdf_doc.w - Gives Width of the Page
    pdf_doc.set_x(0)
